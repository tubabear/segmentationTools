...
class InteractiveImageSegmentation:
    ...
    def process(self, img):
        ...
        while True:
            ...
            alpha = 0.8 if self.draw_color==0 else (1 if self.draw_color==1 else 0)
...
if __name__ == '__main__':
    ...
    fimglist = sorted([x for x in os.listdir(img_dir) if '.png' in x or '.jpg' or "JPG" in x])
    idx = 0
    ...
    while idx<len(fimglist):
        ...
        if key == ord('s') or key == 10:
            img = cv2.imread(os.path.join(img_dir,fimg))
            w, h = img.shape[:2]
            iis.mask = cv2.resize(iis.mask, (h, w), interpolation=cv2.INTER_NEAREST_EXACT)
